...
| `Mjlab-Jump-Flat-MicroDuck` | flat | Two-foot hop, land standing |
...
uv run scripts/infer_policy.py --walking walk.onnx --standing stand.onnx \
    --sitstand sitstand.onnx --roulade roulade.onnx --jump jump.onnx --new-cmd-obs
```

Keyboard-driven (velocity commands, `G` ground pick, `Y` sit/stand, `R` roulade,
`K`/`L` kicks, `J` jump); `--debug`, `--save-csv`, `--record` support sim2real
comparisons. Jump auto-returns after ~2 s (it ends standing).
...
| `robot_allcollisions.xml` | VelStand, StandUp, SitStand, GroundPick, BallKick, Roulade, Jump (body can physically lie on the ground) |
