...
class PolicyInference:
    def __init__(self, model, data, walking_onnx_path=None, action_scale=1.0,
        ...
        # Episodic behavior policies (kick, roulade, jump). All use the unified
        # 61D obs layout with an ALL-ZERO 13D command (twist forced ~0 in
        # training, head/body slots zero-padded), so triggering one is a plain
        # session swap; after `duration` seconds control hands back to
        ...
        if (
            ...
            and "jump" not in self.behavior_sessions
        ):
            raise ValueError(
                "At least one of --walking, --standing, --sitstand or --jump must be provided"
            )
    ...
    def trigger_behavior(self, name):
        """Start an episodic behavior (kick_left / kick_right / roulade / jump).
...
def main():
    ...
    parser.add_argument("--jump", type=str, default=None, help="Path to jump (two-foot hop) policy ONNX (press J to trigger). Requires --new-cmd-obs.")
    ...
    print("  J:                jump / two-foot hop (requires --jump)")
